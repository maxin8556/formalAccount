...
class AccountsAccess:
    ...
    def forbidden_account(self, file_path):
        ...
        # print(orignal_data)
        # print(type(orignal_data))
        # aes_encrypt = AESEncrypt(AES_KEY)
        orignal_data = CryptoTool().decrypt(orignal_data)
        orignal_data = str(orignal_data, 'utf-8')
        orignal_data = JsonTool().str_to_dict(orignal_data)

        orignal_data.update({"enable": False})

        # 加密写入文件
        orignal_data = CryptoTool().encrypt_from_dict_data(orignal_data)
        # print(orignal_data)
        # print(type(orignal_data))
        orignal_data = str(orignal_data, 'utf-8')
        # print(orignal_data)
...
class CustomSocialActions(TwitterApiAccess):
    ...
    def tweet_with_like(self, path, account_info):
        ...
        # print(type(account_info))
        # aes_encrypt = AESEncrypt(AES_KEY)
        aes_encrypt = CryptoTool().decrypt(account_info)
        aes_encrypt = str(aes_encrypt, 'utf-8')
        account_info = JsonTool().str_to_dict(aes_encrypt)
        ...
        if not process_object:
            ...
            if not process_object.set_cookies(task["cookies"]):
                ...
                return 3, "账号Cookies不可用，禁用账号文件：{}".format(task["task_file"])
