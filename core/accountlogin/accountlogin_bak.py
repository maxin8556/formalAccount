    ...
    def twitter_login(self, account_info, **kwargs):
        ...
        try:
            ...
            if self._driver.current_url == 'https://twitter.com/home':
                ...
                # return self._driver.get_cookies(), True, ""
            ...
            if 'https://twitter.com/login/error' in self._driver.current_url:
                if "The username and password you entered did not match our records. Please double-check and try again" in err_str:
                    ...
                    # return None, False, "账号登录失败，{}账号密码错误再进行确认".format(account_info.get("name"))
            ...
            if 'https://twitter.com/login/check' in self._driver.current_url:
                ...
                # return None, False, "账号登录失败，不存在该账号{}".format(account_info.get("name"))
                return None, 1, "账号登录失败，不存在该账号{}".format(account_info.get("name"))

            # if 'https://twitter.com/login/check' or 'https://twitter.com/login/error' in self._driver.current_url:
                # if "The username and password you entered did not match our records. Please double-check and try again" in err_str:
                    # return None, False, "账号登录失败，{}账号密码错误再进行确认".format(account_info.get("name"))
                # else:
                    # self._logger.info(u'账号登录失败，用户=【{}】'.format(account_info.get("name")))
                    # return None, False, "账号登录失败，不存在该账号{}".format(account_info.get("name"))
            ...
            if account_info["account"]:
                ...
                # return None, False, "账号登录失败，该账号不存在手机号码{}".format(account_info.get("name"))
    ...
    def account_login_with_cookies_return(self, account_info):
        ...
        if 0 == status and cookies:
            ...
            JsonTool.write_json_file(record,
                                     "account-" + account_info["name"] + "-" + datetime.datetime.now().strftime('%Y%m%d_%H%M%S') + ".json",
                                     self.save_path)
            self._logger.info("有有效cookie为{}，状态码为{}".format(cookies,status))
            return 0, ""
        elif 2 == status:
            return 2, "账号封停"
        elif 3 == status:
            ...
            self._logger.error("没有有效cookie为{}，状态码为{}".format(cookies,status))
            return 1, "账号不存在"
